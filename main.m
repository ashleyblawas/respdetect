...
% gm18_239a
%p = [p1(1:10*35253); p2(10*35253+1:10*46534); p3(10*46534+1:10*50350);  p4(10*50350+1:10*54331); p5(10*54331+1:10*59142);...
...
all_breath_locs.breath_idx = [all_breath_locs.breath_idx; val3; diff_vals_ps]; %diff_vals_jp];
all_breath_locs.type = [all_breath_locs.type; repmat("jsp", length(val3), 1); repmat("ps", length(diff_vals_ps), 1)];% repmat("jp", length(diff_vals_jp), 1)];
...
[all_breath_idx{j}] = get_breathidx(all_dive_dur{j}, all_si_breathtimes{j});

% Get number of breaths during surface interval as a count
[all_surf_breath_count{j}] = get_breathcounts(all_dive_dur{j}, all_si_breathtimes{j}, all_breath_idx{j});

% Get surface interval durations
[breathing_dur] = get_breathdur(all_dive_dur{j}, all_si_breathtimes{j}, all_breath_idx{j});
...
for i = 1:length(all_dive_dur)
    subplot(5, 8, i)
    scatter(all_si_breathtimes{i}, all_fR{i}, 10, id_cols(i, :), 'filled', 'MarkerFaceAlpha',0.8,'MarkerEdgeAlpha', 1); hold on
    box on; axis square;
end
...
fig = figure
for i = 1:length(all_dive_dur)
    scatter(all_si_breathtimes{i}, all_fR{i}, 10, id_cols(i, :), 'filled', 'MarkerFaceAlpha',0.5,'MarkerEdgeAlpha', 0.5); hold on
    box on; axis square;
end
...
for k = 1:length(all_dive_dur)
    for i = 1:length(all_dive_dur{k})-1
        if i < length(all_dive_dur{k})-1
            scatter(all_si_breathtimes{k}(all_breath_idx{k}(i):all_breath_idx{k}(i+1)-1), all_fR{k}(all_breath_idx{k}(i):all_breath_idx{k}(i+1)-1), 5, ones(1, length(all_breath_idx{k}(i):all_breath_idx{k}(i+1)-1)).*all_dive_dur{k}(i),'filled', 'MarkerFaceAlpha',0.25,'MarkerEdgeAlpha', 0.25);
            hold on; box on; 
        elseif i == length(all_dive_dur{k})-1
            scatter(all_si_breathtimes{k}(all_breath_idx{k}(i):end), all_fR{k}(all_breath_idx{k}(i):end), 5, ones(1, length(all_fR{k}(all_breath_idx{k}(i):end))).*all_dive_dur{k}(i),'filled', 'MarkerFaceAlpha',0.25,'MarkerEdgeAlpha',0.25);
            hold on; box on;
        end
...
%% Plot pre and post-dive average breathing rates
for k = 1:length(all_dive_dur)
    tag = taglist{k};
    ...
    % Load in dive table
    load(strcat(data_path, "\diving\divethres_5m\", metadata.tag, "divetable"))
    Tab = table2array(Tab(:, 2:12));
    
    pre5_dive_fR{k}(1) = NaN;
    pre3_dive_fR{k}(1) = NaN;
    pre1_dive_fR{k}(1) = NaN;
    post5_dive_fR{k}(length(Tab(:, 1))) = NaN;
    post3_dive_fR{k}(length(Tab(:, 1))) = NaN;
    post1_dive_fR{k}(length(Tab(:, 1))) = NaN;
    
    for i = 2:length(Tab(:, 1)) %Not the first dive because we don't know if we had all of the surface interval before it
        pre5_dive_breaths{k}{i-1} = all_breath_times{k}(find(all_breath_times{k}>Tab(i, 3)-5*60 & all_breath_times{k}<Tab(i, 3) & all_breath_times{k}>Tab(i-1, 4)));
        pre5_dive_fR{k}(i) = mean(60./diff(pre5_dive_breaths{k}{i-1}));
    end
    clear i
    
    for i = 2:length(Tab(:, 1)) %Not the first dive because we don't know if we had all of the surface interval before it
        pre3_dive_breaths{k}{i-1} = all_breath_times{k}(find(all_breath_times{k}>Tab(i, 3)-3*60 & all_breath_times{k}<Tab(i, 3) & all_breath_times{k}>Tab(i-1, 4)));
        pre3_dive_fR{k}(i) = mean(60./diff(pre3_dive_breaths{k}{i-1}));
    end
    clear i
    
    for i = 2:length(Tab(:, 1)) %Not the first dive because we don't know if we had all of the surface interval before it
        pre1_dive_breaths{k}{i-1} = all_breath_times{k}(find(all_breath_times{k}>Tab(i, 3)-1*60 & all_breath_times{k}<Tab(i, 3) & all_breath_times{k}>Tab(i-1, 4)));
        pre1_dive_fR{k}(i) = mean(60./diff(pre1_dive_breaths{k}{i-1}));
    end
    clear i
    
    for i = 1:length(Tab(:, 1))-1
        post5_dive_breaths{k}{i} = all_breath_times{k}(find(all_breath_times{k}<Tab(i, 4)+5*60 & all_breath_times{k}<Tab(i+1, 3) & all_breath_times{k}>Tab(i, 4)));
        post5_dive_fR{k}(i) = mean(60./diff(post5_dive_breaths{k}{i}));
    end
    clear i
    
    for i = 1:length(Tab(:, 1))-1
        post3_dive_breaths{k}{i} = all_breath_times{k}(find(all_breath_times{k}<Tab(i, 4)+3*60 & all_breath_times{k}<Tab(i+1, 3) & all_breath_times{k}>Tab(i, 4)));
        post3_dive_fR{k}(i) = mean(60./diff(post3_dive_breaths{k}{i}));
    end
    clear i
    
    for i = 1:length(Tab(:, 1))-1
        post1_dive_breaths{k}{i} = all_breath_times{k}(find(all_breath_times{k}<Tab(i, 4)+1*60 & all_breath_times{k}<Tab(i+1, 3) & all_breath_times{k}>Tab(i, 4)));
        post1_dive_fR{k}(i) = mean(60./diff(post1_dive_breaths{k}{i}));
    end
    clear i
...
subplot(2, 3, 1)
for k = 1:length(all_dive_dur)
    scatter(all_dive_dur{k}, pre5_dive_fR{k}, 24, id_cols(k, :), 'filled', 'MarkerFaceAlpha', 1);
    hold on;
    temp_mat = [cell2mat(all_dive_dur); cell2mat(pre5_dive_fR)]';
    temp_mat(any(isnan(temp_mat),2),:) = []; 
    c = polyfit(temp_mat(:, 1),temp_mat(:, 2),1);
    y_est = polyval(c,linspace(min(temp_mat(:, 1)), max(temp_mat(:, 1)), 1000));
    plot(linspace(min(temp_mat(:, 1)), max(temp_mat(:, 1)), 1000) ,y_est,'k:','LineWidth',2)
    text(20, 20 ,sprintf('m=%f',c(1)))
end
box on; axis square;
xl1 = xlim; yl1 = ylim;
...
subplot(2, 3, 2)
for k = 1:length(all_dive_dur)
    scatter(all_dive_dur{k}, pre3_dive_fR{k}, 24, id_cols(k, :), 'filled', 'MarkerFaceAlpha', 1);
    hold on;
    temp_mat = [cell2mat(all_dive_dur); cell2mat(pre3_dive_fR)]';
    temp_mat(any(isnan(temp_mat),2),:) = []; 
    c = polyfit(temp_mat(:, 1),temp_mat(:, 2),1);
    y_est = polyval(c,linspace(min(temp_mat(:, 1)), max(temp_mat(:, 1)), 1000));
    plot(linspace(min(temp_mat(:, 1)), max(temp_mat(:, 1)), 1000) ,y_est,'k:','LineWidth',2)
...
subplot(2, 3, 3)
for k = 1:length(all_dive_dur)
    scatter(all_dive_dur{k}, pre1_dive_fR{k}, 24, id_cols(k, :), 'filled', 'MarkerFaceAlpha', 1);
    hold on;
    temp_mat = [cell2mat(all_dive_dur); cell2mat(pre1_dive_fR)]';
    temp_mat(any(isnan(temp_mat),2),:) = []; 
    c = polyfit(temp_mat(:, 1),temp_mat(:, 2),1);
    y_est = polyval(c,linspace(min(temp_mat(:, 1)), max(temp_mat(:, 1)), 1000));
    plot(linspace(min(temp_mat(:, 1)), max(temp_mat(:, 1)), 1000) ,y_est,'k:','LineWidth',2)
...
subplot(2, 3, 6)
for k = 1:length(all_dive_dur)
    scatter(all_dive_dur{k}, post1_dive_fR{k}, 24, id_cols(k, :), 'filled', 'MarkerFaceAlpha', 1);
    hold on;
    temp_mat = [cell2mat(all_dive_dur); cell2mat(post1_dive_fR)]';
    temp_mat(any(isnan(temp_mat),2),:) = []; 
    c = polyfit(temp_mat(:, 1),temp_mat(:, 2),1);
    y_est = polyval(c,linspace(min(temp_mat(:, 1)), max(temp_mat(:, 1)), 1000));
    plot(linspace(min(temp_mat(:, 1)), max(temp_mat(:, 1)), 1000) ,y_est,'k:','LineWidth',2)
...
subplot(2, 3, 5)
for k = 1:length(all_dive_dur)
    scatter(all_dive_dur{k}, post3_dive_fR{k}, 24, id_cols(k, :), 'filled', 'MarkerFaceAlpha', 1);
    hold on;
    temp_mat = [cell2mat(all_dive_dur); cell2mat(post3_dive_fR)]';
    temp_mat(any(isnan(temp_mat),2),:) = []; 
    c = polyfit(temp_mat(:, 1),temp_mat(:, 2),1);
    y_est = polyval(c,linspace(min(temp_mat(:, 1)), max(temp_mat(:, 1)), 1000));
    plot(linspace(min(temp_mat(:, 1)), max(temp_mat(:, 1)), 1000) ,y_est,'k:','LineWidth',2)
...
subplot(2, 3, 4)
for k = 1:length(all_dive_dur)
    scatter(all_dive_dur{k}, post5_dive_fR{k}, 24, id_cols(k, :), 'filled', 'MarkerFaceAlpha', 1);
    hold on;
    temp_mat = [cell2mat(all_dive_dur); cell2mat(post5_dive_fR)]';
    temp_mat(any(isnan(temp_mat),2),:) = []; 
    c = polyfit(temp_mat(:, 1),temp_mat(:, 2),1);
    y_est = polyval(c,linspace(min(temp_mat(:, 1)), max(temp_mat(:, 1)), 1000));
    plot(linspace(min(temp_mat(:, 1)), max(temp_mat(:, 1)), 1000) ,y_est,'k:','LineWidth',2)
