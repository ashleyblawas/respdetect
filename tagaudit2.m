function     [RES,AXs,hhh] = tagaudit2(tag,tcue,RES, j, DMON)
%
...
while 1,
   ...
   %kk = 1:NS*fs;
   ks = kb + round(tcue*fs) ;
   axes(AXm), plot(tt(tcue*fs+kk),j(tcue*fs+kk), 'k-'), grid
   ...
   if ~isempty(p),
      kk = 1:5:NS*fs;
      ks = kb + round(tcue*fs) ;
      axes(AXp), plot(tt(tcue*fs+kk),p(tcue*fs+kk),'-'), grid
      %plot(ks/fs,p(ks)), grid
   ...
   while done == 0,
       ...
       elseif button=='p',
           ...
           if ~isempty(bs),
               ...
               end_time = length(xf)/(afs/SOUND_DF);
               h=line([0,0],get(gca,'YLim'),'color','r','marker', '.', 'linewidth', 2);
               pause(1)
               sound(volume*xf,afs/SOUND_DF,16);
               tic % Starts Matlab timer
               t=toc; % Gets the time since the timer started
               while t<end_time
                   set(h, 'xdata', t+tcue*[1,1]) % Moves the line to the time indicated by t
                   drawnow % necessary to get figure updated
                   t=toc; % get the current time for the next update
               end
           else
               end_time = length(x)/(afs/SOUND_DF);
               h=line([tcue,tcue],get(gca,'YLim'),'color','r','marker', '.', 'linewidth', 1);
               pause(1)
      ...
      elseif button=='s', %Go to next surface interval
          ...
          endidx = length(p);
          val = find(p(startidx:endidx)<5, 1);
