function load_data(data_path, speciesCode)
...
% Stanford University
    ...
    prhFolder = fullfile(data_path, speciesCode, 'prh');
    ...
    for i = 1:length(flds)
        if not(isfolder(strcat(data_path, speciesCode, '\', flds(i))))
            mkdir(strcat(data_path, speciesCode, '\', flds(i)));
        end
    end
