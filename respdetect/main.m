...
Rnew = breath_audit(taglist{k}, tcue, R, date_on, p_tag, jerk_filt(start_idx:end_idx), rad2deg(roll(start_idx:end_idx)));%, rad2deg(pitch(start_idx:end_idx))); %Run audit (for d3s)
[Rnew.cue, order] = sort(Rnew.cue);
Rnew.stype = Rnew.stype(order);
breath_saveaudit(strcat(data_path, '\breaths\', INFO.whaleName, 'breaths'), Rnew); % Save audit
...
tagon_datetime_1hour = tagon_datetime + hours(1);

breaths_datetime_after1hour = breaths.cue(breaths.cue > tagon_datetime_1hour);

tagon_duration_after1hour = date(find(tagon ==1, 1, 'last'))-date(find(tagon ==1, 1, 'first'))-hours(1);

fr_overall = length(breaths_datetime_after1hour)/minutes(tagon_duration_after1hour);

%% Step 7e: Make table and write to csv

tag = {INFO.whaleName};
num_breaths = length(breaths_datetime_after1hour);
tag_duration = tagon_duration_after1hour;
fr = fr_overall;
...
%% Step S2c: Plot detections accuracy after acoustic validation
