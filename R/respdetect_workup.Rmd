---
title: "Acoustic validation for respdetect"
...
ggplot() +  
  #geom_rect(aes(xmin=-2, xmax=131, ymin=0, ymax=5), alpha = 0.1) +
 # geom_segment(data=matched_breaths, aes(x = seq(1, length(audited_breath_start)), y = diff_start, xend = seq(1, length(audited_breath_start)), yend = diff_end)) +
  geom_point(data=matched_breaths, aes(x = seq(1, length(audited_breath_start)), y =diff, color = tag)) + 
  #geom_point(data=matched_breaths, aes(x = seq(1, length(audited_breath_start)), y = diff_end))+
...
pre_dive_surfs_longdives = pre_dive_surfs %>% filter(cluster == 1)
post_dive_surfs_longdives = post_dive_surfs %>% filter(cluster == 1)
...
plot_grid(top_row, p2, labels = c('', 'C'), ncol = 1, rel_widths = c(1,2.25), label_size = 12)
...
p2 = ggplot(data = post_shortdives_breaths, aes(x = breath_surf_idx, y = fr, color = dive_dur_m))+
  ...
  theme_bw() + scale_color_viridis(limits = c(0 , 25), name = "Dive Duration (min)")

plot_grid(p1, p2,  labels = c('A', 'B'), rel_widths = c(1,1.6), label_size = 12)
