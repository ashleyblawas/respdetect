...
dir = "C:/Users/ashle/Dropbox/Ashley/Graduate/Manuscripts/respdetect/tests/gm"
...
# Randomly pull rows and add their duration until you get to a total of X% of total logging time

# Edit this to be the percentage of logging time you want to audit for audible breaths (a decimal from 0-1)
percent_to_pull = 0.10
n = floor(total_log_time_s)
...
while(total_sampled_dur <= n){
  ...
  if(dim(plyr::match_df(sample_temp, logs_to_audit))[1]==0){
...
```{r}
list_of_files = list.files(path = paste(dir, '/audit', sep = ""),
...
audited_breaths = audited_breaths %>% mutate(tag = str_sub(str_sub(FileName,-25),1,9))
...
ggplot() +  
  geom_point(data=matched_breaths, aes(x = seq(1, length(audited_breath_start)), y = diff), color = "black", size = 2) + 
  xlab('Index') + 
  ylab(expression('Difference between audited \n breath and detected breath (s)')) + 
  theme_bw() + 
  ylim(0, 5.2) + 
  geom_hline(yintercept  = 5, linetype = 2) +
  scale_x_continuous(limits=c(0, 55), breaks=seq(0,55,5)) + 
  theme(axis.title.y = element_text(vjust=2)) + 
  theme(axis.title.y = element_text(margin = margin(t = 0, r = 0, b = 0, l = 20)))


ggsave(paste(dir, "acousticval.jpg"), dpi = 300, width = 5, height = 3)
...
rm(dive_dur, xx, clustering, h.cl)
```
